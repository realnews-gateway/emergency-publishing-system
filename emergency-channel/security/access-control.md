...
### Policy Management
- **Policy-as-Code**: Store RBAC policies as code where supported.
- **Review Process**: Require PRs for policy changes with automated policy linting and mandatory reviewers from @Empus/security.
- **Change Control**: Maintain a changelog of policy updates with rationale and risk assessment.
...
- **mTLS**: Prefer mTLS for high-sensitivity channels; validate CN/SAN and expected audience.
- **Token Validation**: Validate tokens at the application layer: issuer, audience, expiry, and revocation status.
...
- **Emergency Process**: Define a documented break-glass process for emergency access.
- **Requirements**:
...
- **Post-Incident**: Break-glass events must trigger a post-incident review and remediation plan.

---

## Secrets and Credential Management

- **Storage**: Store secrets in an approved secrets manager or KMS; never commit secrets to source control.
- **Isolation**: Use environment-scoped secrets and avoid sharing secrets across services.
- **Rotation**: Enforce secret rotation policies and automated secret scanning in CI.
- **Retrieval**: Limit secret access to the minimal set of principals and log all secret retrievals.
...
- **Policy Changes**: PRs that widen privileges require:
...
- **Emergency Changes**: Emergency policy changes may be applied with post-facto review and must be documented.

---

## Auditing and Monitoring

- **Scope**: Audit all access control changes and privileged operations.
- **Metadata**: Records must include actor, role, action, resource, justification, and timestamp.
- **Security Alerts**: Alert on anomalous access patterns: mass privilege grants, repeated failed elevation attempts, unusual token usage.
...
- **Fail Closed**: Fail closed on authorization failures; do not silently degrade to permissive behavior.

---

## CI and Automation Controls

- **CI Identity**: CI systems must use ephemeral credentials and least-privilege service accounts.
- **Security Gates**: PRs modifying IAM policies must trigger policy-as-code linting and secret scanning.
- **Policy Enforcement**: Prevent merges that introduce wildcard or overly broad permissions.
